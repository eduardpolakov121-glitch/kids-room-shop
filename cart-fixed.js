...
function addToCart(id) {
    ...
    animateAddToCart(id, product.img);
...
function animateAddToCart(id, fallbackImg) {
    const cartButton = document.querySelector(".cart-button");
    if (!cartButton) return;

    const productCard = document.getElementById("prod-" + id);
    const productImage = productCard?.querySelector("img") || document.querySelector("#product-page img");
    const sourceRect = productImage?.getBoundingClientRect();
    const targetRect = cartButton.getBoundingClientRect();

    if (!sourceRect || !targetRect || sourceRect.width === 0 || sourceRect.height === 0) {
        pulseCartButton();
        return;
    }

    const flyer = document.createElement("img");
    flyer.src = productImage?.currentSrc || productImage?.src || fallbackImg || "";
    flyer.alt = "";
    ...
    flyer.style.boxShadow = "0 10px 28px rgba(0,0,0,0.22)";
    flyer.style.transition = "transform 0.75s cubic-bezier(.22,.61,.36,1), opacity 0.75s ease, left 0.75s cubic-bezier(.22,.61,.36,1), top 0.75s cubic-bezier(.22,.61,.36,1), width 0.75s ease, height 0.75s ease";

    document.body.appendChild(flyer);

    const targetSize = Math.max(26, Math.min(targetRect.width, targetRect.height) * 0.45);
    const targetLeft = targetRect.left + targetRect.width / 2 - targetSize / 2;
    const targetTop = targetRect.top + targetRect.height / 2 - targetSize / 2;
    ...
    requestAnimationFrame(() => {
        ...
        flyer.style.transform = "scale(0.35) rotate(12deg)";
    });
